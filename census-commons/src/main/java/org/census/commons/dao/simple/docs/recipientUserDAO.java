...
public class recipientUserDAO extends Connectors {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                do {
                    ...
                    simpleEmployee = this.employeesDao.findById((long) rs.getInt("appointeduserid"));

                    if (simpleEmployee != null)
                        // Фамилия И.О. пользователя-который поручил служебку
                        recipient.setAppointedUserName(simpleEmployee.getShortRusName());
     ...
     */
    public List NotMake(daoHandler dH, long DeptID, long personnelId) {
