package org.census.commons.utils.report;
...
public final class TextTable {
    ...
    private static final Log log = LogFactory.getLog(TextTable.class);
    ...
    */
    ...
    /***/
    // todo: create 'excel export config' class for configuring exported to excel report
    public void exportToExcelFile(String fileName, String dataSheetName, boolean isBoldHeaders) throws IOException {
        ...
        if (!StringUtils.isBlank(fileName)) { // file name is OK
            ...
            // table model data
            String     metaHeader = this.tableModel.getMetaHeader(); //
            String[]   header     = this.tableModel.getHeader();     // table header row
            String[][] data       = this.tableModel.getData();       // table data rows

            // creating excel file model
            HSSFWorkbook wb    = new HSSFWorkbook();        // Excel 97-2007 file (xls)
            HSSFSheet    sheet = wb.createSheet(StringUtils.isBlank(dataSheetName) ? DEFAULT_SHEET_NAME : dataSheetName); // new sheet
            ...
            // --- writing META HEADER
            if (!StringUtils.isBlank(metaHeader) && header.length > 0) {

                // select meta header cells style
                // todo: duplicate code in meta header and header!
                HSSFCellStyle style;
                if (isBoldHeaders) {
                    // header cells style
                ...
                HSSFRow headerRow = sheet.createRow((short) rowsCounter); // row for meta header
                Cell metaHeaderCell = headerRow.createCell(0);
                metaHeaderCell.setCellStyle(style);
                metaHeaderCell.setCellValue(metaHeader);
                sheet.addMergedRegion(new CellRangeAddress(rowsCounter, rowsCounter, 0, header.length));
                rowsCounter++;
            }
            ...
            if (header.length > 0) { // put header data to excel (if there is data)
                ...
                HSSFRow headerRow = sheet.createRow((short) rowsCounter);     // new row (header row), rows are zero-based
                for (String headerValue : header) {
                    Cell newCell = headerRow.createCell(headerCellsCounter);
                    newCell.setCellValue(headerValue);
                    newCell.setCellStyle(style);
                    // todo: call this at the end of processing whole sheet (performance)
                    sheet.autoSizeColumn((short) headerCellsCounter); // autosize every column to fit the text
                    headerCellsCounter++; // increment header cells (columns) counter
                }
                ...
                for (String[] dataRow : data) {
                    ...
                    if (dataRow != null) { // add data from data row
                        for (String cellValue : dataRow) {
                            ...
                            newDataCell.setCellStyle(dataStyle);
    ...
    public static void main(String[] args) throws IOException {
        Log log = LogFactory.getLog(TextTable.class);
        ...
        System.out.println("->\n" + table.getTextGrid());
        table.exportToExcelFile("c://temp//report.xls", "ZZZ", true);
