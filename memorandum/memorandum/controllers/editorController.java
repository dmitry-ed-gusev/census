       ...
       if (memoID > 0){
           ...
           if (depts != null){
               ...
               if (emailsList != null && emailsList.size() > 0){
                   ...
                   // Тема письма
                   JMConfig.setSubject("Переадресация СЗ № " + memo.getMemoNumber() + " от " + simpleEmployee.getShortName()
                           + " (" + simpleEmployee.getDepartmentCode() + ")");

                   // Текст письма
                   String msgText = "Вам была переадресована служебная записка №" + memo.getMemoNumber()
                           + " от " +  simpleEmployee.getShortName() + " (" + simpleEmployee.getDepartmentCode() + ")" + "\n";
   ...
   // которая отображает результат добавления новой служебки
   else if (ANSWER_APPOINTED_MEMO_RESULT_ACTION.equals(ACTION))
    {
     ...
     if ((answer != null) && (!answer.trim().equals("")) && (recipient != null)) {
         ...
         recipientUpdate.setAnswer(answer + "(исп. " + simpleEmployee.getShortName() + ")");
         ...
         if (recipient.getSendemail() == 1) {
             ...
             if (appointedUserEmails != null) {
                 ...
                 String msgText = "Служебная записка № " + memo.getMemoNumber() + " выполнена: " + simpleEmployee.getShortName() + ".\n";
                 ...
                 if (newMemoID > 0) {
                     ...
                     msgText = msgText + "Касательно: " + memo.getSubject() + "\n";
                     msgText = msgText + "Выполнил: " + simpleEmployee.getShortName() + "\n";
