...
insert into CENSUS_AUTH_SCHEMA.auth_user(id, name, description, username, password)
    values (1, 'Test user', 'Test system user', 'test', 'test');
insert into CENSUS_AUTH_SCHEMA.auth_user(id, name, description, username, password)
    values (2, 'Dmitrii Gusev', 'Fisrt system user', 'dgusev', '123456');

-- adding some roles
insert into CENSUS_AUTH_SCHEMA.auth_role(id, rolename, description)
    values (1, 'USER', 'System user without special privileges.');
insert into CENSUS_AUTH_SCHEMA.auth_role(id, rolename, description)
    values (2, 'ADMIN', 'System admin. Privileged user.');
